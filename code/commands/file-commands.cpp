	...
	for (;;) {
		...
		try{
			...
			for (gchar* i = text; *i; ++i) {
				...
				gchar* next_pos = i;
				++next_pos;
				last_node->add_child_text(Glib::ustring(last_pos, next_pos - last_pos));
				last_pos = next_pos;
...
void dump_info(xmlpp::Element* node, DocWindow& document) {
	...
	char buf[n];
  {
	...
	// TODO: do some more magic to make localising this easier
	node->add_child_text(_("Document generated by "));
	xmlpp::Element* link = node->add_child("a");
	link->set_attribute("href", "http://gobby.0x539.de/");
	link->add_child_text(PACKAGE_STRING);
	node->add_child_text(
		Glib::ustring::compose(_(" %1 from %2 at %3"),
	  	PACKAGE_STRING, session_info, time_str));
}
...
void add_user_colours(xmlpp::Element* css,
	...
	for (std::set<InfTextUser*>::const_iterator i = users.begin();
		...
		gdouble r, g, b;
		// TODO: use Gdk::Color c; c.set_hsv(...) if we are not depending on 2.14
		gtk_hsv_to_rgb(hue, 0.35, 1.0, &r, &g, &b);
		unsigned int rgb =
			 ((static_cast<unsigned int>(r * 0xff) << 16)
			| (static_cast<unsigned int>(g * 0xff) << 8)
			| (static_cast<unsigned int>(b * 0xff))) & 0xffffff;
		gchar const* name = inf_user_get_name(INF_USER(*i));
		// TODO: figure out if we can get "written by <name>" tooltips easily
		std::ostringstream ss;
		ss
			<< ".user_" << id << " {\n"
			<< "  background-color:       #" << std::hex << rgb << ";\n"
			<< "}\n";

		css->add_child_text(ss.str());
